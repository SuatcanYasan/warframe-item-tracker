...
import { ARCANES, ARCANE_MAX_COPIES } from "../../constants/arcanes";
...
function EmptyCTA({ label, onClick }: EmptyCTAProps) {
  return (
...
function PreviewRow({ items, totalCount, moreLabel }: PreviewRowProps) {
...
interface CraftPerItem {
  uniqueName: string;
  requirements?: CraftRequirement[];
}

interface AlmostDoneItem {
  name: string;
  img?: string | null;
  done: number;
  total: number;
...
export default function DashboardPage() {
  ...
  const primaryColor = (useAppStore((s) => s.customThemeTokens?.colorPrimary) as string | undefined) || "#CA8A04";

  const selectedItems = useCraftStore((s) => s.selectedItems) as SelectedCraftItem[];
  const completedMap = useCraftStore((s) => s.completedMap) as Record<string, Record<string, number>>;
  const calculation = useCraftStore((s) => s.calculation) as { totals?: any[]; perItem?: CraftPerItem[] };
  ...
  const craftStats = useMemo(() => {
    ...
    const done = totals.filter((r: any) => {
      const completed = ((completedMap as any)[r.uniqueName] as number) || 0;
  ...
  const craftPreviews = useMemo<PreviewItem[]>(() =>
    selectedItems.slice(0, 3).map((i) => ({ name: i.name, img: i.imageUrl })),
  [selectedItems]);

  const invPreviews = useMemo<PreviewItem[]>(() => {
    const parts = Object.values(inventoryParts);
  ...
  const almostDone = useMemo<AlmostDoneItem[]>(() => {
    if (!calculation.perItem) return [];
    return selectedItems
      .map((item): AlmostDoneItem | null => {
        ...
        const done = reqs.requirements.filter((r) => (((completedMap as any)[r.uniqueName] as number) || 0) >= (r.totalNeeded || r.quantity || 1)).length;
        if (total === 0 || done === total) return null;
        const pct = Math.round((done / total) * 100);
        if (pct < 40) return null;
        return { name: item.name, img: item.imageUrl, done, total, pct };
      })
      .filter((x): x is AlmostDoneItem => x !== null)
  ...
  const lastAddedTime = useRelativeTime(lastAdded?.addedAt ?? 0);
  ...
  const [tipIdx, setTipIdx] = useState<number>(() => Math.floor(Math.random() * 5));
  useEffect(() => {
  ...
  const [timerData, setTimerData] = useState<TimersData | null>(null);
  useEffect(() => {
    requestJson("/api/timers").then((d) => setTimerData(d as TimersData)).catch(() => {});
    const iv = setInterval(() => requestJson("/api/timers").then((d) => setTimerData(d as TimersData)).catch(() => {}), 60000);
    return () => clearInterval(iv);
  }, []);
  ...
  useEffect(() => {
    if (anyExpired) requestJson("/api/timers").then((d) => setTimerData(d as TimersData)).catch(() => {});
  }, [anyExpired]);

  function formatCountdown(expiry: string | undefined | null): string {
    if (!expiry) return "—";
  ...
  const timerSlides = useMemo<TimerSlide[]>(() => {
    if (!timerData) return [];
    const slides: TimerSlide[] = [];
  ...
  const incCurrent = useMemo(() => {
    ...
    const claimed = group.weapons.filter((w) => (incClaimedMap as any)[`${group.key}:${w.name}`]).length;
  ...
  const wfCurrent = useMemo(() => {
    ...
    const claimed = group.warframes.filter((w) => (wfClaimedMap as any)[`${group.key}:${w.name}`]).length;
  ...
  return (
    <div className="dashboard-page">
      ...
      {/* Hero — Craft */}
      <motion.div className="dashboard-card dashboard-hero" style={{ ["--accent" as any]: "var(--wf-primary)" }} initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} onClick={() => navigate("/craft")}>
      ...
      <div className="dashboard-grid-secondary">
        <motion.div className="dashboard-card" style={{ ["--accent" as any]: "#a855f7" }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.12 }} onClick={() => navigate("/relic")}>
        ...
        <motion.div className="dashboard-card" style={{ ["--accent" as any]: "#3b82f6" }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18 }} onClick={() => navigate("/inventory")}>
        ...
        <motion.div className="dashboard-card" style={{ ["--accent" as any]: "#f59e0b" }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.24 }} onClick={() => navigate("/mastery")}>
      ...
      <div className="dashboard-grid-features">
        {/* MR XP */}
        <motion.div className="dashboard-feature-card" style={{ ["--accent" as any]: "#f59e0b" }} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.30 }} onClick={() => navigate("/mastery")}>
        ...
        {/* Incarnon Bu Hafta */}
        <motion.div className="dashboard-feature-card" style={{ ["--accent" as any]: "#fbbf24" }} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.34 }} onClick={() => navigate("/incarnon")}>
          <div className="feature-card-header">
            <span className="feature-card-emoji">⚡</span>
        ...
        <motion.div className="dashboard-feature-card" style={{ ["--accent" as any]: "#60a5fa" }} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.38 }} onClick={() => navigate("/warframe-rotation")}>
          <div className="feature-card-header">
        ...
        {/* Arcanes */}
        <motion.div className="dashboard-feature-card" style={{ ["--accent" as any]: "#a855f7" }} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.42 }} onClick={() => navigate("/arcanes")}>
          <div className="feature-card-header">
            <span className="feature-card-emoji">🔮</span>
