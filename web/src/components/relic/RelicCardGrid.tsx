...
const rarityColors: Record<string, string> = { Common: "default", Uncommon: "green", Rare: "blue", Legendary: "gold" };
...
interface GroupedRelic {
  baseName: string;
  rarity?: string;
  byRefinement: Record<string, number | undefined>;
}
...
function parseRefinement(location: string): string {
  const match = location.match(/\((\w+)\)$/);
  return match ? match[1] : "Intact";
}

function getBaseRelicName(location: string): string {
  return location.replace(/\s*\(\w+\)$/, "");
}

function groupDropsByRelic(drops: Drop[]): GroupedRelic[] {
  const grouped = new Map<string, GroupedRelic>();
...
interface RelicPrimeCardProps {
  prime: SelectedCraftItem;
  foundMap: Record<string, boolean>;
  onToggleFound: (uniqueName: string, componentName: string) => void;
  onRemove: (prime: SelectedCraftItem) => void;
  onOpenModal: (prime: SelectedCraftItem) => void;
}

function RelicPrimeCard({ prime, foundMap, onOpenModal }: RelicPrimeCardProps) {
  const { t } = useTranslate();
  const { data: dropData } = useItemDrops(prime.uniqueName);
  const componentDrops: ComponentDrop[] = (dropData as DropData)?.componentDrops || [];
...
function RelicDetailModal({ prime, open, onClose, foundMap, onToggleFound }: RelicDetailModalProps) {
  const { t } = useTranslate();
  const [componentActiveKeys, setComponentActiveKeys] = useState<string | string[]>([]);
  const [refinementLevel, setRefinementLevel] = useState<string>("Intact");

  const { data: dropData, isLoading: loadingDrops } = useItemDrops(open ? prime?.uniqueName ?? null : null);
        ...
        <>
          <div style={{ marginBottom: 12 }}>
            ...
            <Segmented
              ...
              onChange={(v) => setRefinementLevel(String(v))}
...
interface Props {
  ...
  foundComponents: Record<string, Record<string, boolean>>;
  onToggleFound: (uniqueName: string, componentName: string) => void;
  onRemovePrime: (prime: SelectedCraftItem) => void;
}

export default function RelicCardGrid({ watchedPrimes, foundComponents, onToggleFound, onRemovePrime }: Props) {
  const { t } = useTranslate();
  const [modalPrime, setModalPrime] = useState<SelectedCraftItem | null>(null);
