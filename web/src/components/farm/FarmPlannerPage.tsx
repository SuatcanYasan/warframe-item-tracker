...
function parsePlace(place: string | null | undefined): ParsedPlace {
...
interface DropInfoModalProps {
  resource: TrackedResource | null;
  onClose: () => void;
  t: TranslateFn;
}

function DropInfoModal({ resource, onClose, t }: DropInfoModalProps) {
  const open = !!resource;
  const { data: drops, isLoading } = useQuery<DropEntry[] | undefined>({
...
interface LocationRowProps {
  loc: CombinedLocation;
  t: TranslateFn;
}

function LocationRow({ loc, t }: LocationRowProps) {
  return (
...
export default function FarmPlannerPage() {
  ...
  const trackedResources = useFarmStore((s) => s.trackedResources) as TrackedResource[];
  ...
  const [infoFor, setInfoFor] = useState<TrackedResource | null>(null);
  const [showCaches, setShowCaches] = useState<boolean>(true);

  const { data: resourceSearch } = useQuery<{ results?: SearchResult[] }>({
  ...
  const dropQueries = useQueries({
    queries: trackedResources.map((r) => ({
      ...
      queryFn: () => requestJson(`/api/drops/search/${encodeURIComponent(r.name)}`) as Promise<DropEntry[]>,
      staleTime: 30 * 60 * 1000,
      retry: 0,
    })),
  });
  ...
  const combinedLocations = useMemo<CombinedLocation[]>(() => {
    const byLocation = new Map<string, { place: ParsedPlace; byResource: Map<string, LocationResource> }>();
    dropQueries.forEach((q, i) => {
      const resource = trackedResources[i];
      if (!resource || !Array.isArray(q.data)) return;
      for (const d of q.data as DropEntry[]) {
        const p = parsePlace(d.place);
        const entry = byLocation.get(p.key) || { place: p, byResource: new Map<string, LocationResource>() };
                  ...
                  <div className="farm-resource-body">
                    ...
                    <div className="farm-resource-target-row">
                      ...
                      <InputNumber
                        ...
                        max={9999}
                        value={r.target}
                        onChange={(v) => updateTarget(r.uniqueName, v ?? 1)}
                        size="small"
                        style={{ width: 80 }}
                      />
                    </div>
