...
import type { EnrichedRequirement } from "../../utils/helpers";

interface AddedAtLabelProps {
  timestamp: number;
}

function AddedAtLabel({ timestamp }: AddedAtLabelProps) {
...
interface SortableItemCardProps {
  item: SelectedCraftItem;
  index: number;
  enrichedByItem: Map<string, EnrichedRequirement[]>;
  onOpenDetail: (item: SelectedCraftItem) => void;
  onRemoveItem: (item: SelectedCraftItem) => void;
  t: TranslateFn;
...
function SortableItemCard({ item, index, enrichedByItem, onOpenDetail, onRemoveItem, t, sortingDisabled, multiSelectMode, isMultiSelected, onToggleMulti }: SortableItemCardProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
...
export default function ItemCardGrid({ items, enrichedByItem, onOpenDetail, onRemoveItem, onOpenSearchDrawer }: Props) {
