...
interface Props {
  item: SelectedCraftItem | null;
  open: boolean;
  onClose: () => void;
  enrichedRequirements: EnrichedRequirement[] | undefined;
  onSetCompleted: (itemUniqueName: string, req: EnrichedRequirement, val: number | null) => void;
  onUpdateQuantity: (uniqueName: string, val: number | null) => void;
}
      ...
      <div className="detail-req-grid">
        <AnimatePresence>
          {reqs.map((req, i) => (
            ...
            >
              <img
                src={(req as any).imageUrl || FALLBACK_ICON}
                alt={req.name}
