...
type MasteryStatus = "owned" | "gilded" | undefined;
...
interface AmpCardProps {
  ...
  onCycle: (uniqueName: string) => void;
  t: TranslateFn;
}

function AmpCard({ part, status, onCycle, t }: AmpCardProps) {
...
interface SectionProps {
  title: string;
  parts: AmpPart[];
  masteryParts: Record<string, MasteryStatus>;
  onCycle: (uniqueName: string) => void;
  t: TranslateFn;
}

function Section({ title, parts, masteryParts, onCycle, t }: SectionProps) {
  return (
...
export default function AmpMasteryTab({ allParts }: Props) {
  const { t } = useTranslate();
  const masteryParts = useAmpStore((s) => s.masteryParts) as Record<string, MasteryStatus>;
  ...
  function handleCycle(uniqueName: string) {
