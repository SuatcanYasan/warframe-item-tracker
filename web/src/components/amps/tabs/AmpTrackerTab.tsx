...
import { Button, Tooltip, Modal, Tag, Spin, Empty as AntEmpty } from "antd";
import {
  CheckOutlined,
  DeleteOutlined,
  InfoCircleOutlined,
...
interface FullPart {
  ...
  components?: Array<{ uniqueName?: string; name: string; itemCount?: number }>;
}

function resolveImage(slotPart: AmpPartRef | null | undefined, allParts: FullPart[]): string {
...
interface SetCardProps {
  ...
  onTogglePart: (id: string, slot: string) => void;
  t: TranslateFn;
}

function SetCard({ set, allParts, onRemove, onTogglePart, t }: SetCardProps) {
  const parts = [
...
export default function AmpTrackerTab({ allParts, onGoToBuilder }: Props) {
  const { t } = useTranslate();
  const trackedSets = useAmpStore((s) => s.trackedSets) as unknown as TrackedSet[];
  ...
  const [dropModalFor, setDropModalFor] = useState<MaterialModalState | null>(null);
  ...
  function handleToggle(setId: string, slot: string) {
    const content = document.querySelector(".app-content") as HTMLElement | null;
    const scrollTop = content?.scrollTop || 0;
    togglePartDone(setId, slot as "prism" | "scaffold" | "brace");
  ...
  const neededMaterials = useMemo(() => {
    const map = new Map<string, { uniqueName?: string; name: string; itemCount: number }>();
    for (const s of trackedSets) {
      for (const slot of ["prism", "scaffold", "brace"] as const) {
        const part = s[slot];
        ...
        for (const comp of full.components) {
          ...
          const prev = map.get(key) || { uniqueName: comp.uniqueName, name: comp.name, itemCount: 0 };
          map.set(key, {
  ...
  const materialUns = useMemo(
    () => neededMaterials.map((m) => m.uniqueName).filter(Boolean) as string[],
    [neededMaterials],
  );
  const { data: materialMeta } = useQuery<any>({
  ...
  const metaByUn: Record<string, any> = materialMeta?.itemsByUniqueName || {};
        ...
        <div className="amp-builder-materials amp-tracker-materials">
          <div className="amp-builder-materials-title">
            ...
            <span className="amp-tracker-materials-count">
              {neededMaterials.filter((m) => m.uniqueName && completedMaterials[m.uniqueName]).length}/{neededMaterials.length}
            </span>
          </div>
          ...
          <div className="amp-builder-materials-grid">
            {neededMaterials.map((m) => {
              const img = (m.uniqueName && metaByUn[m.uniqueName]?.imageUrl) || FALLBACK_ICON;
              const done = !!(m.uniqueName && completedMaterials[m.uniqueName]);
              return (
                <div
...
interface MaterialDropModalProps {
  material: MaterialModalState | null;
  onClose: () => void;
  t: TranslateFn;
}

function MaterialDropModal({ material, onClose, t }: MaterialDropModalProps) {
  const open = !!material;
  ...
  const drops: DropEntry[] = ((data?.drops as DropEntry[]) || []).slice().sort((a, b) => (b.chance || 0) - (a.chance || 0));
