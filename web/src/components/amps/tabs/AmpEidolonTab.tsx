...
interface ShieldCalcResult {
  seconds: number;
  dps: number;
}

function calcShieldTime(amp: AmpStats | null | undefined, shieldHP: number): ShieldCalcResult | null {
  if (!amp) return null;
...
export default function AmpEidolonTab({ ampsData }: Props) {
  const { t } = useTranslate();
  const [prismUn, setPrismUn] = useState<string | null>(null);

  const allPrisms = useMemo<AmpStats[]>(
    () => [
          ...
          return (
            <div key={e.key} className="amp-eidolon-card">
              <img
                ...
                onError={(ev) => { (ev.target as HTMLImageElement).src = FALLBACK_ICON; }}
                loading="lazy"
                decoding="async"
              />
