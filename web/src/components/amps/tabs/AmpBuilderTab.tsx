...
interface MaterialEntry extends AmpComponent {
  itemCount: number;
}
...
function buildCode(prism?: AmpPart, scaffold?: AmpPart, brace?: AmpPart): string {
  const n = (p?: AmpPart) => (p ? (p.number === 0 ? "M" : p.number) : "?");
...
const BRACE_BONUSES: Record<string, string[]> = {
...
function mergeMaterials(parts: AmpPart[]): MaterialEntry[] {
...
export default function AmpBuilderTab({ ampsData }: Props) {
  const { t } = useTranslate();
  const addTrackedSet = useAmpStore((s) => s.addTrackedSet);
  const trackedSets = useAmpStore((s) => s.trackedSets);

  const [prismUn, setPrismUn] = useState<string | null>(null);
  const [scaffoldUn, setScaffoldUn] = useState<string | null>(null);
  const [braceUn, setBraceUn] = useState<string | null>(null);

  const allPrisms = useMemo<AmpPart[]>(
    () => [...(ampsData?.mote?.prism ? [ampsData.mote.prism] : []), ...(ampsData?.prisms || [])],
    [ampsData],
  );
  const allScaffolds = useMemo<AmpPart[]>(
    () => [...(ampsData?.mote?.scaffold ? [ampsData.mote.scaffold] : []), ...(ampsData?.scaffolds || [])],
    [ampsData],
  );
  const allBraces = useMemo<AmpPart[]>(
  ...
  const selected = [prism, scaffold, brace].filter(Boolean) as AmpPart[];
  ...
  const materialUns = useMemo(
    () => materials.map((m) => m.uniqueName).filter(Boolean) as string[],
    [materials],
  );
  ...
  const firingParts = [prism, scaffold].filter(Boolean) as AmpPart[];
  ...
  const renderOption = (part: AmpPart) => ({
  ...
  const alreadyTracked = useMemo(
    () => trackedSets.some((s: any) => s.code === code),
    [trackedSets, code],
  );
      ...
      {materials.length > 0 && (
        <div className="amp-builder-materials">
          ...
          <div className="amp-builder-materials-grid">
            {materials.map((m) => {
              const meta = m.uniqueName ? metaByUn[m.uniqueName] : undefined;
