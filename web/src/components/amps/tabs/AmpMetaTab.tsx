...
function findPart(ampsData: any, name: string, slot: "prism" | "scaffold" | "brace"): AmpPart | null {
...
interface PresetCardProps {
  ...
  ampsData: any;
  t: TranslateFn;
  onTrack: (preset: MetaPreset, prism: AmpPart | null, scaffold: AmpPart | null, brace: AmpPart | null) => void;
  alreadyTracked: boolean;
}

function PresetCard({ preset, ampsData, t, onTrack, alreadyTracked }: PresetCardProps) {
...
export default function AmpMetaTab({ ampsData }: Props) {
  const { t } = useTranslate();
  const trackedSets = useAmpStore((s) => s.trackedSets) as Array<{ code: string }>;
  const addTrackedSet = useAmpStore((s) => s.addTrackedSet);
  const trackedCodes = new Set(trackedSets.map((s) => s.code));

  function trackPreset(preset: MetaPreset, prism: AmpPart | null, scaffold: AmpPart | null, brace: AmpPart | null) {
