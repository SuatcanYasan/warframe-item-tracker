...
function useCountdown(expiry: string | number | Date | null | undefined): string | null {
...
function CycleCard({ title, icon, stateLabel, expiry, color, totalMin, accentClass, bgImage }: CycleCardProps) {
...
interface TimersData {
  cetus?: { state?: string; isDay?: boolean; expiry?: string };
  vallis?: { state?: string; isWarm?: boolean; expiry?: string };
  cambion?: { state?: string; expiry?: string };
  voidTrader?: { activation?: string; expiry?: string; location?: string };
}

export default function TimersPage() {
  const { t } = useTranslate();
  const [data, setData] = useState<TimersData | null>(null);
  const [loading, setLoading] = useState<boolean>(true);

  function fetchTimers() {
    setLoading(true);
    requestJson("/api/timers")
      .then((d) => setData(d as TimersData))
      .catch(() => {})
  ...
  const baroActive = !!baro && new Date(baro.activation || 0).getTime() <= Date.now() && new Date(baro.expiry || 0).getTime() > Date.now();
