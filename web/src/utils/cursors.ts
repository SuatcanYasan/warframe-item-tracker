function lighten(hex: string, amt = 48): string {
...
function cursor(svg: string, hotspot: string): string {
  const encoded = encodeURIComponent(svg).replace(/'/g, "%27").replace(/"/g, "%22");
  return `url("data:image/svg+xml,${encoded}") ${hotspot}`;
}

export function generateCursorVars(primaryColor: string): Record<string, string> {
...
export function applyCursorVars(primaryColor: string): void {
