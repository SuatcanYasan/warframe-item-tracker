...
export type WarframeRole = "DPS" | "Tank" | "Support" | "Stealth";
...
export interface WFGroup {
  key: string;
  warframes: WarframeEntry[];
}

export const WF_GROUPS: readonly WFGroup[] = [
  {
    key: "A",
    warframes: [
...
export interface CurrentWFRotation {
  groupIndex: number;
  weeksFromEpoch: number;
  mondayUtc: number;
  nextMonday: number;
}

export interface UpcomingWFRotation {
  group: WFGroup;
  weekStart: number;
  isCurrent: boolean;
}

export function getCurrentRotation(now: number = Date.now()): CurrentWFRotation {
...
export function getUpcomingRotations(weeksAhead: number = 8, now: number = Date.now()): UpcomingWFRotation[] {
  const { groupIndex, mondayUtc } = getCurrentRotation(now);
  const result: UpcomingWFRotation[] = [];
