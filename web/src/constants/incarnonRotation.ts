...
export interface IncarnonGroup {
  key: string;
  weapons: IncarnonWeapon[];
}

export const INCARNON_GROUPS: readonly IncarnonGroup[] = [
  {
    key: "A",
    weapons: [
...
export function getCurrentRotation(now: number = Date.now()): CurrentRotation {
...
export function getUpcomingRotations(weeksAhead: number = 8, now: number = Date.now()): UpcomingRotation[] {
  const { groupIndex, mondayUtc } = getCurrentRotation(now);
  const result: UpcomingRotation[] = [];
