...
import { enrichRequirements, type EnrichedRequirement, type Requirement } from "../utils/helpers";
import type { SelectedCraftItem } from "../types";

interface PerItemEntry {
  uniqueName: string;
...
export interface AdjustedTotal extends TotalEntry {
  remaining: number;
...
export interface CraftDerivedBag {
  detailByItem: Map<string, Requirement[]>;
  enrichedByItem: Map<string, EnrichedRequirement[]>;
...
export function useCraftDerived(): CraftDerivedBag {
  ...
  const detailByItem = useMemo(() => {
    ...
    for (const item of (calculation.perItem || []) as PerItemEntry[]) {
  ...
  const enrichedByItem = useMemo(() => {
    const map = new Map<string, EnrichedRequirement[]>();
  ...
  const adjustedTotals = useMemo<AdjustedTotal[]>(() => {
    ...
    return ((calculation.totals || []) as TotalEntry[]).map((total) => {
      ...
      const completionPercent =
        total.quantity > 0
          ...
          : 100;
      const status: AdjustedTotal["status"] =
...
export function useFilteredSelectedItems(
  enrichedByItem: Map<string, EnrichedRequirement[]>,
): SelectedCraftItem[] {
  const selectedItems = useCraftStore((s) => s.selectedItems);
...
export function useFilteredTotals(adjustedTotals: AdjustedTotal[]): AdjustedTotal[] {
